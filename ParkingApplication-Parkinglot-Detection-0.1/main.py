...
fn_yaml = r"CUHKSquare2.yml"
fn_out = r"output4.avi"
...
print(cap.isOpened()) #debug to see if video is open
...
print("w: " + str(video_info['width']) + "h: " + str(video_info['height']))
...
if config['save_video']:
    ...
    print("Debug:  codec defined")  # debug showing if fired off
...
with open(fn_yaml, 'r') as stream:
    ...
    print("Debug:  yaml loaded")  #debug showing yaml file loaded.
    
#parking_contours = []  #Didn't go anywhere - DT
parking_bounding_rects = []
parking_mask = []
for ind, park in enumerate(parking_data):
    points = np.array(park['points'])
    print("Debug: points shape")
    print(points.shape)
    rect = cv2.boundingRect(points)
    points_shifted = points.copy()
    points_shifted[:,0] = points[:,0] - rect[0] # shift contour to roi
    points_shifted[:,1] = points[:,1] - rect[1]
    #parking_contours.append(points) #Didn't go anywhere - DT
...
print(cap.isOpened())  #debug to check if video still open
while(cap.isOpened()): 
    # Read frame-by-frame    
    ...
    if config['parking_detection']:        
        for ind, park in enumerate(parking_data):
            print("Debug: park in parking data")
            print(park)
            points = np.array(park['points'])
            rect = parking_bounding_rects[ind]
            print("Debug: park bounding rects")
            print(parking_bounding_rects[ind])
            roi_gray = frame_gray[rect[1]:(rect[1]+rect[3]), rect[0]:(rect[0]+rect[2])] # crop roi for faster calcluation
            #roi_gray = np.zeros((rect[3], rect[2]), dtype=np.uint8)
    ...
    if config['parking_overlay']:
        print("Debug: Parking Overlay fired off")
        for ind, park in enumerate(parking_data):
            points = np.array(park['points'])
